...
if (Test-Path "$directory\rubymine.nuspec") {
    Write-Host "Found cached rubymine.nuspec"
    $oldVersion = (([xml](Get-Content "$directory\rubymine.nuspec")).package.metadata.version)
} else {
    Write-Host "Couldn't find cached rubymine.nuspec at $directory\rubymine.nuspec"
    $oldVersion = "0.0.0"
}
...
$majorVersion = $release.RM.majorVersion
...
if (([version]$oldVersion -lt [version]$newVersion) -or $force) {
    Write-Host "Cached rubymine.nuspec not found or web version differs from cache"
...
try {
    ...
    $release_url = "https://confluence.jetbrains.com/display/RUBYDEV/RubyMine+$($majorVersion)"
